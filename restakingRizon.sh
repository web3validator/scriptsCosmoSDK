...
for (( ;; )); do
        ...
        echo -e "${PASWD}\n${PASWD}\n" | rizond tx distribution withdraw-rewards ${VALIDATOR} --chain-id groot-011 --from ${ACC_NAME} --node ${NODE} --commission -y --fees 5000uatolo
        ...
        echo -e "BALANCE: ${GREEN}${BAL}${NC} uatolo\n"
        echo -e "Stake ALL\n"
        echo -e "${PASWD}\n${PASWD}\n" | rizond tx staking delegate ${VALIDATOR} ${BAL}uatolo --chain-id groot-011 --from ${ACC_NAME} --node ${NODE} -y --fees 5000uatolo
